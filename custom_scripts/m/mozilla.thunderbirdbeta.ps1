...
if ($result.LATEST_THUNDERBIRD_DEVEL_VERSION -gt $package.last_checked_tag)
{
    $update_found = $true
    $version = $result.LATEST_THUNDERBIRD_DEVEL_VERSION
    $jsonTag = $result.LATEST_THUNDERBIRD_DEVEL_VERSION
    $locales = @('en-US','en-GB')
