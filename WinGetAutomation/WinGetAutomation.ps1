...
Function Confirm-VersionAlreadyExists {
    ...
    Param (
        ...
        [Parameter(Mandatory = $true, ParameterSetName = 'CheckRepo')]
        [System.Management.Automation.SwitchParameter] $CheckRepo,

        [Parameter(Mandatory = $true, ParameterSetName = 'CheckPRs')]
    ...
    If ($CheckRepo) {
        ...
        $ExistingPRs = gh search prs --repo=microsoft/winget-pkgs --match=title --state=open `
            --json='author,isDraft,title,url' author:vedantmgoyal9 author:SpecterShell $PackageIdentifier.Replace('.', ' ') $PackageVersion | `
    ...
    for ($_Index = 0; $_Index -lt $Formula.Source.Length; $_Index++) {
        ...
        If ($Formula.Source[$_Index].Headers -is [System.Management.Automation.PSCustomObject]) {
            $Formula.Source[$_Index].Headers.PSObject.Properties | ForEach-Object `
                ...
                -Process { 
                If (($_.Value -eq 'Bearer $env:GITHUB_BOT_TOKEN' -and -not $DoNotAddGitHubAuthHeaders) -or $_.Value -ne 'Bearer $env:GITHUB_BOT_TOKEN') {
...
Function Update-Manifest {
    ...
    If (-not $DryRun) {
        If (Confirm-VersionAlreadyExists -PackageIdentifier $UpdateInfo.PackageIdentifier -PackageVersion $UpdateInfo.PackageVersion -CheckPRs) {
    ...
    & $script:WingetCreateExe update $UpdateInfo.PackageIdentifier `
        --version $UpdateInfo.PackageVersion `
        --urls $UpdateInfo.InstallerUrls `
        --out $script:ManifestsDir `
        --token $script:GithubBotToken
    ...
    $UpdateInfo.PSObject.Properties | Where-Object {
        ...
        If ($_.Name -eq 'Locales') {
            $_.Value.ForEach({
                    $LocaleName = $_.Name
                    $LocaleManifestPath = (Get-ChildItem -Path $script:ManifestsDir -File -Recurse).Where({ $_.Name -match $LocaleName }).FullName
            ...
            $InstallerManifestPath = (Get-ChildItem -Path $script:ManifestsDir -File -Recurse).Where({ $_.Name -match 'installer' }).FullName
    ...
    If (-not $DryRun) {
        ...
        & $script:WingetCreateExe submit (Get-ChildItem -Path $script:ManifestsDir -Directory -Recurse | Select-Object -Last 1 -ExpandProperty FullName) --token $env:WINGETCREATE_TOKEN

        # Remove manifests after submission
        Remove-Item -Path $script:ManifestsDir -Recurse -Force
    }
...
If ($script:isCIandGitHubActions) {
    ...
    $script:GithubBotToken = $env:GITHUB_BOT_TOKEN = Get-GitHubBotToken
}
...
'@
    ...
    # This is to ensure that the formula path is resolved correctly in GitHub Actions as <job>.defaults.run.working-directory is
    ## set to ./WinGetAutomation (steps are run in this directory, rather than the repository root) and,
    ## tj-actions/changed-files action gives output relative to the repository root
    $FormulaToTest = (Resolve-Path -Path $FormulaToTest -RelativeBasePath ($script:isCIandGitHubActions ? '..': '.')).Path
    ...
    $ManifestsPath = Get-ChildItem -Path $script:ManifestsDir -Recurse -Directory | Select-Object -Last 1 -ExpandProperty FullName
    $Manifests = ''
    ...
    If ($Null -eq $ManifestsPath -or $ManifestContent -eq '') {
        $Manifests = @'
...
(Get-ChildItem -Path $PSScriptRoot\Formula\ -File -Recurse).FullName | ForEach-Object -ThrottleLimit 5 -Parallel {
    $FormulaPath = $_
    ...
    If ($UpdateInfo._WinGetAutomation.UpdateRequired) {
        ...
        try {
            ...
            ($using:ErrorUpgradingPkgs).Add("- **$($UpdateInfo.PackageIdentifier)**: $($_.Exception.Message)`n```````n$StdOutLog`n```````n")
        }
...
(gh issue view 900 --json comments | ConvertFrom-Json).comments
.Where({ $_.author.login -eq 'vedantmgoyal-bot' })
.ForEach({ gh api --method DELETE /repos/vedantmgoyal9/vedantmgoyal9/issues/comments/$($_.url.Substring($_.url.IndexOf('-') + 1)) })
