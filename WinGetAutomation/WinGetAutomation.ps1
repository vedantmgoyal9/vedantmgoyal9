...
Function Confirm-VersionAlreadyExists {
    [OutputType([System.Boolean], ParameterSetName = 'CheckRepo')]
    [OutputType([System.Management.Automation.PSObject], ParameterSetName = 'GetExistingPRs')]
    Param (
        ...
        [Parameter(Mandatory = $true, ParameterSetName = 'GetExistingPRs')]
        [System.Management.Automation.SwitchParameter] $GetExistingPRs
    )
    ...
    } ElseIf ($GetExistingPRs) {
        return $(gh search prs --repo=microsoft/winget-pkgs --match=title --state=open `
...
Function Update-Manifest {
    ...
    If (-not $DryRun) {
        $ExistingPRs = Confirm-VersionAlreadyExists -PackageIdentifier $UpdateInfo.PackageIdentifier -PackageVersion $UpdateInfo.PackageVersion -GetExistingPRs
    ...
    If (-not $DryRun -and $UpdateInfo.PSObject.Properties.Where({ $_.Name -in @('ProductCode', 'AppsAndFeaturesEntries', 'Locales') }).Count -ge 1) {
        ...
        & $script:WingetCreateExe submit $ManifestsPath --token $env:WINGETCREATE_TOKEN
    }
}
#endregion
