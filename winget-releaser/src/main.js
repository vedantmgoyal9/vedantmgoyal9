...
core.info(
  'Checking whether the user has forked winget-pkgs repository or not...'
)
...
core.info(
  'Checking whether the package already exists in winget-pkgs repository or not...'
)
https.get(
  'https://github.com/microsoft/winget-pkgs/tree/master/manifests/' +
    pkgid.charAt(0).toLowerCase() +
    '/' +
    pkgid.replace('.', '/'),
  (res) => {
    if (res.statusCode !== 200) {
...
exec.exec('Invoke-WebRequest', [
  '-Uri',
  'https://github.com/microsoft/winget-cli/releases/download/v1.1.12701/9c0fe2ce7f8e410eb4a8f417de74517e_License1.xml',
  '-OutFile',
  'license.xml',
])
exec.exec('Import-Module', ['-Name', 'Appx', '-UseWindowsPowerShell'])
exec.exec('Add-AppxProvisionedPackage', [
  '-Online',
  '-PackagePath',
  '.\\winget.msixbundle',
...
exec.exec('explorer.exe', [
  '"shell:appsFolder\\Microsoft.DesktopAppInstaller_8wekyb3d8bbwe!winget"',
])
...
async function cloneRepo() {
  await exec.exec(
    ...
    [
      'clone',
      'https://' + usr + ':' + pat + '@github.com/microsoft/winget-pkgs.git',
      '--quiet',
    ],
    options
  )
  await exec.exec('git', ['remote', 'rename', 'origin', 'upstream'], {
    cwd: 'winget-pkgs',
    options,
  })
  await exec.exec(
    ...
    [
      ...
      'add',
      'origin',
      'https://github.com/' + usr + '/winget-pkgs.git',
    ],
    { cwd: 'winget-pkgs', options }
  )
  return null
}
cloneRepo()
    ...
    fs.readdirSync(lastver).forEach((file) => {
      if (/\.installer\.yaml$/g.test(file)) {
        ...
        inst_manifest.Installers.sort((a, b) => {
          return a.InstallerUrl < b.InstallerUrl
            ? -1
            : a.InstallerUrl > b.InstallerUrl
            ? 1
            : 0
        })
        ...
        fs.writeFileSync(
          path.join(pkg_dir, pkgver, path.basename(file)),
          yaml.safeDump(inst_manifest, { lineWidth: -1 }),
          'utf8'
        )
        ...
        fs.writeFileSync(
          path.join(pkg_dir, pkgver, path.basename(file)),
          yaml.safeDump(locale_manifest, { lineWidth: -1 }),
          'utf8'
        )
      } else {
        ...
        fs.writeFileSync(
          path.join(pkg_dir, pkgver, path.basename(file)),
          yaml.safeDump(version_manifest, { lineWidth: -1 }),
          'utf8'
        )
      }
    })
...
async function submitPR() {
  ...
  await exec.exec(
    'git',
    ['commit', '-m', 'New Version: ' + pkgid + 'version ' + pkgver],
    { cwd: 'winget-pkgs', options }
  )
