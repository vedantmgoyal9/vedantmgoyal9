...
New-Item -Name manifests -Path . -ItemType Directory -Force # Create manifests directory
$SingleManifestsDir = Convert-Path -Path .\manifests\ # Get full path of manifests directory
...
ForEach ($i in $UpgradeObject) {
    ...
    ForEach ($Version in $VersionsToPatch) {
        ...
        for ($iteration = 0; $iteration -lt $InstallersManifest.Installers.Count; $iteration++) {
            ...
            docker run --rm -v ${SingleManifestsDir}:C:\working-dir\ get-arp-winget
