...
Invoke-WebRequest -Uri https://github.com/vedantmgoyal2009/Komac/releases/download/nightly/Komac-nightly.jar -OutFile .\komac.jar # Download latest version of Komac
...
ForEach ($Upgrade in $UpdateInfo_First21Pkgs) {
    ...
    try {
        ...
        Invoke-Expression -Command "& $env:JAVA_HOME_17_X64\bin\java.exe -jar $CmdToRun" -ErrorAction Stop
    } catch {
...
$ErrorUpgradingPkgs += (($PreviousComments | Select-Object -Last 1).body | Select-String -Pattern '-\s.*\s(?=\r?\n)' -AllMatches).Matches.Value
$ErrorUpgradingPkgs = $ErrorUpgradingPkgs | Sort-Object -Unique
$CommentBody = @"
...
**Error while upgrading packages:** $(
        If ($ErrorUpgradingPkgs.Count -gt 0) {
            "$($ErrorUpgradingPkgs.Count) package manifests were not submitted.`n$($ErrorUpgradingPkgs -join "`n")"
        } Else {
