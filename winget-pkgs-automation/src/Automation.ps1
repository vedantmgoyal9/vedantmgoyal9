...
ForEach ($Upgrade in $UpgradeObject) {
    ...
    # Clear variable since it doesn't find next package's last versions, we're dot-sourcing YamlCreate.ps1
    Remove-Variable -Name LastVersion -Force -ErrorAction SilentlyContinue
}
