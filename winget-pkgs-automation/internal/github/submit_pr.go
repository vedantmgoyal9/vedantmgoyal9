...
func SubmitManifests(pkg_id, version string, manifests []WinGetManifest, commitType CommitType, forkUser string, rmReason ...string) {
	...
	pullRequestBody.WriteString("### Pull request has been created with ")
	if os.Getenv("WGPKGSCLI_TOOL") != "" {
		pullRequestBody.WriteString(os.Getenv("WGPKGSCLI_TOOL"))
	} else {
		pullRequestBody.WriteString("[wgpkgs](https://github.com/vedantmgoyal2009/vedantmgoyal2009/blob/main/wgpkgs-cli) (winget-pkgs cli)")
	}
	pullRequestBody.WriteString(" 🪶🪟📦\n")
