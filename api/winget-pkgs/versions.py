...
class handler(BaseHTTPRequestHandler):
  def do_GET(self):
    ...
    if "?" in self.path:
        ...
        self.end_headers()
        self.wfile.write("package_identifier is either empty or not provided".encode())
        return
    ...
    self.send_header('Content-type', 'application/json')
    self.end_headers()
    self.wfile.write(dumps({ 'PackageIdentifier': pkg_id, 'Versions': result[pkg_id] }).encode())
    return
