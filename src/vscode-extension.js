...
function activate(context) {
    ...
    console.log('Congratulations, your extension "vedantmgoyal2009" is now active!');

    // The command has been defined in the package.json file
    // Now provide the implementation of the command with  registerCommand
    // The commandId parameter must match the command field in package.json
    let disposable = vscode.commands.registerCommand('vedantmgoyal2009.helloWorld', function () {
        // The code you place here will be executed every time your command is executed

        // Display a message box to the user
        vscode.window.showInformationMessage('Hello World from vedantmgoyal2009 extension!');
    });

    let manageWpaPkgs = vscode.commands.registerCommand('vedantmgoyal2009.manageWpaPackages', async function () {
        let pkgList = new Map();
        let dirContents = await vscode.workspace.findFiles(`src/winget-pkgs-automation/packages/**/*.json`)
        ...
        vscode.window.showQuickPick(Array.from(pkgList.keys()).sort(), {
            ...
            vscode.window.showQuickPick([
                ...
                if (selectedAction.includes('open')) {
                    ...
                    vscode.workspace.fs.readFile(vscode.Uri.file(`${vscode.workspace.workspaceFolders[0].uri.path}/src/winget-pkgs-automation/schema.json`)).then(schemaDoc => {
