$result = Invoke-WebRequest -Uri "https://sourceforge.net/projects/$($package.repo_uri)/best_release.json" -UseBasicParsing | ConvertFrom-Json
if ($result.platform_releases.windows.date -gt $package.last_checked_tag)
{
    ...
    $prefix = "http://downloads.sourceforge.net/project/$($package.repo_uri)/"
    $newPrefix = "https://sourceforge.net/projects/$($package.repo_uri)/files/"
    $suffix = "\?.*"
    $newSuffix = "/download"
