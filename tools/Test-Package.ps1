...
.LINK
    https://github.com/vedantmgoyal2009/vedantmgoyal2009/blob/main/tools/Test-Package.ps1
#>
...
Process {
    ...
    $Package = Get-Content -Path "$PSScriptRoot\..\src\winget-pkgs-automation\packages\$($PackageIdentifier.Substring(0,1).ToLower())\$($PackageIdentifier.ToLower()).json" -Raw -ErrorAction SilentlyContinue | ConvertFrom-Json
    If ($Null -eq $Package) {
