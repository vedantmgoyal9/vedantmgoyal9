...
Begin {
    ...
    Function Test-Package {
        ...
        If ($_IsValid -eq $true) {
            ...
            Write-Error "Some values are missing or empty, please fix the json file manually and run:`n   .\Test-Package.ps1 -PackageIdentifier $($Package.Identifier)"
        }
    }
}
...
End {
    Write-Output 'Do you want to create/test another package?'
    If (Get-UserInput -Method KeyPress -Message 'Choice (y/n): ' -ReturnValues @{ Y = $true; N = $false }) {
        & $PSCommandPath
    }
}